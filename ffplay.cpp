...
static VideoState* stream_open(const char* filename, AVInputFormat* iformat)
{
    ...
    if (!is->filename)
        AfxMessageBox(L"Failed to get filename.");
        goto fail;
    is->iformat = iformat;
    is->ytop = 0;
    is->xleft = 0;
    ...
    if (frame_queue_init(&is->pictq, &is->videoq, VIDEO_PICTURE_QUEUE_SIZE, 1) < 0) {
        AfxMessageBox(L"Failed to init video frame queue.");
        goto fail;
    }
    if (frame_queue_init(&is->subpq, &is->subtitleq, SUBPICTURE_QUEUE_SIZE, 0) < 0) {
        AfxMessageBox(L"Failed to init subtitle frame queue.");
        goto fail;
    }
    if (frame_queue_init(&is->sampq, &is->audioq, SAMPLE_QUEUE_SIZE, 1) < 0) {
        AfxMessageBox(L"Failed to init audio frame queue.");
        goto fail;
    }
    if (packet_queue_init(&is->videoq) < 0 || packet_queue_init(&is->audioq) < 0 || packet_queue_init(&is->subtitleq) < 0) {
        AfxMessageBox(L"Failed to init packet queue.");
        goto fail;
    }

    if (!(is->continue_read_thread = SDL_CreateCond())) {
        // av_log(NULL, AV_LOG_FATAL, "SDL_CreateCond(): %s\n", SDL_GetError());
        AfxMessageBox(L"SDL_CreateCond() error.");
        goto fail;
    }
    ...
    if (!is->read_tid) {
        ...
        AfxMessageBox(L"SDL_CreateThread() error.");
    fail:
        stream_close(is);
        return NULL;
    }
    ...
    */
    ...
    if (!display_disable) {
        ...
        */
        ...
        // window = SDL_CreateWindow(program_name, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 0, 0, flags);
        window = SDL_CreateWindowFrom(dlg->m_display);
        ...
        if (window) {
            ...
            if (!renderer) {
                ...
                AfxMessageBox(L"Failed to initialize a hardware accelerated renderer.");
